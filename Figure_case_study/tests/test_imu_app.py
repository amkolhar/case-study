...
class TestIMUApp(BaseTest):
    ...
    def test_imu_roll_angle(self, req_roll_angle):
        ...
        assert resp_roll_angle-self.roll_err == approx(roll_angle_set, abs=threshold), 
                        f"When Roll angle set to {roll_angle_set}, IMU reported {resp_roll_angle} which is not within the 0.1 degree threshold"
    ...
    def test_imu_roll_angle(self, req_pitch_angle):
        ...
        assert resp_pitch_angle-self.pitch_err == approx(pitch_angle_set, abs=threshold), 
                        f"When Roll angle set to {pitch_angle_set}, IMU reported {resp_pitch_angle} which is not within the 0.1 degree threshold"
